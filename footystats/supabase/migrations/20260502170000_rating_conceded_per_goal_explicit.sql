-- Make "goals conceded" explicit: for each goal the team concedes, every on-pitch player
-- loses rating proportional to minutes (p_share). Penalty per goal is position-weighted
-- (GK/DEF highest) and slightly scales with involvement (impact_factor).
-- Formula: subtract (goals_conceded * p_share * per_goal_penalty) from the raw rating.
-- Bench: p_share = 0 → no per-goal penalty.
...
final as (
  select
    ...
    + c_clean_sheet
    - (p_yellow + p_red)
    -- Negative score from goals conceded: linear in goals_conceded × time on pitch.
    - case
